"""
ПРОФЕССИОНАЛЬНЫЙ СКАНЕР СЕРЕБРА (XAG/USD, фьючерс SI=F).
...
# ----------------- НАСТРОЙКИ -----------------
SYMBOL      = "SI=F"     # серебро. XAG/USD у брокера = почти то же
DEPOSIT_USD = 120.0      # депозит
SPREAD_PCT  = 0.0005     # спред/комиссия за круг по серебру (уточни у брокера)
...
def run_once():
    ...
    print(f"\n{'='*64}")
    print(f"  СЕРЕБРО (XAG/USD)   Цена: {price:.3f}   [{time.strftime('%H:%M:%S')}]")
    print(f"{'='*64}")
...
def main():
    print("ПРОФ-СКАНЕР СЕРЕБРА запущен. Ctrl+C для выхода.")
